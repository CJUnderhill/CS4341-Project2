...
class Board:
    ...
    def make_move(self, column_pos, row_pos, color = "sample",second_move=False):
        ...
        if color in ["white","black"]:
            ...
            print("invalid color")
        if self.board[column_pos + row_pos].play_on_cell(color, second_move1=second_move) == False:
            next_move =  Evaluation.evaluationFunction(self)[1]
            print(next_move)
            x = next_move[0]
            y = next_move[1]
            self.board[self.columns_pos[x-1] + self.rows_pos[y-1]].play_on_cell(color, second_move1=second_move)
            column_pos1 = self.columns_pos[x-1]
            row_pos1 = self.rows_pos[y-1]
            self.last_move = (column_pos1,row_pos1)
    ...
    def check_terminal_state(self):
        x = self.columns_pos.index(self.last_move[0]) + 1
        y = self.rows_pos.index(self.last_move[1]) + 1
        #x = column_index
        #y = row_index
...
class Board_Cell:
    ...
    def check_valid(self, column, row):
        if isinstance(column, str) and isinstance(row, str):
            if is_integer(row):
                row1 = int(row)
                if row1 <= 15 and row1 >= 1 and (column in ["a", "b", "c", "d", "e", "f",
                            "g", "h", "i", "j", "k", "l", "m", "n", "o"]):
