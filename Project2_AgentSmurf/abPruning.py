...
def checkCutOff(currentDepth):
    ...
    if (currentDepth >= MAX_DEPTH) or (nodes_visited >= MAX_NODES_VISITED) or (time.time() - TIME_START >= TIME_STOP):
        #print(time.time() - TIME_START,nodes_visited,currentDepth)
        return True
    else:
        return False
...
def minimaxABPruning(gameBoard):
    global TIME_START, nodes_visited
    nodes_visited = 0
    TIME_START = time.time()
    # Current player
    player = gameBoard.color_turn
    # Make second move
    #if (gameBoard.turns == 1 and
    #    gameBoard.board["H8"].color != gameBoard.our_color and gameBoard.color_turn == gameBoard.our_color):
        # Replace middle piece
     #   gameBoard.last_move = ("H","8")#board["H8"].color = gameBoard.color_turn
    #    return gameBoard #MOVE
#    if gameBoard.turns == 0 and gameBoard.color_turn == gameBoard.our_color:
